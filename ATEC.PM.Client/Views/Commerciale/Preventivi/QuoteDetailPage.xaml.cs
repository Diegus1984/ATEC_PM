...
public partial class QuoteDetailPage : Page
{
    ...
    private async Task LoadQuoteDetail(int quoteId)
    {
        try
        {
            QuoteDto? quote = await ApiClient.GetDataAsync<QuoteDto>($"/api/quotes/{quoteId}");
            ...
            if (!hasAutoIncludes)
            {
                try
                {
                    ...
                    quote = await ApiClient.GetDataAsync<QuoteDto>($"/api/quotes/{quoteId}");
                    if (quote == null) return;
                }
                catch (Exception ex)
                {
                    System.Diagnostics.Trace.WriteLine($"Error reloading auto includes: {ex.Message}");
                }
            }
    ...
    private async Task SaveQuoteTotals(decimal total, decimal profit)
    {
        ...
        {
            System.Diagnostics.Trace.WriteLine($"Error saving quote totals: {ex.Message}");
        }
    }
    ...
    private async Task ChangeStatus(string newStatus)
    {
        ...
        var confirm = ATEC.PM.Client.Controls.ShadcnMessageBox.Show($"Cambiare stato a {newStatus.ToUpperInvariant()}?",
    ...
    private async void BtnConvert_Click(object sender, RoutedEventArgs e)
    {
        ...
        try
        {
            ...
            string json = await ApiClient.PostAsync($"/api/quotes/{_selectedQuoteId}/convert", body);
            if (ConvertQuoteDialog.TryParseConvertResponse(json, out int projectId, out string msg))
            {
                ...
                ATEC.PM.Client.Controls.ShadcnMessageBox.Show(string.IsNullOrEmpty(msg) ? "Errore" : msg);
        }
        catch (Exception ex) { ATEC.PM.Client.Controls.ShadcnMessageBox.Show($"Errore: {ex.Message}"); }
    }
    ...
    private async void BtnDelete_Click(object sender, RoutedEventArgs e)
    {
        ...
        var confirm = ATEC.PM.Client.Controls.ShadcnMessageBox.Show("Eliminare questo preventivo?", "Conferma",
    ...
    private async void BtnPdf_Click(object sender, RoutedEventArgs e)
    {
        ...
        try
        {
            ...
            {
                ATEC.PM.Client.Controls.ShadcnMessageBox.Show("Impossibile generare il PDF", "Errore", MessageBoxButton.OK, MessageBoxImage.Warning);
                return;
            }
            ...
            {
                System.Diagnostics.Trace.WriteLine($"Failed to delete temporary PDF file (likely locked by viewer): {ex.Message}");
            }
    ...
    private async void BtnReloadAutoIncludes_Click(object sender, RoutedEventArgs e)
    {
        ...
        if (ATEC.PM.Client.Controls.ShadcnMessageBox.Show("Ricaricare i contenuti automatici dal catalogo?\nI contenuti automatici attuali verranno sostituiti.",
    ...
    private async void BtnRemoveImpAutoInclude_Click(object sender, RoutedEventArgs e)
    {
        ...
        if (ATEC.PM.Client.Controls.ShadcnMessageBox.Show($"Rimuovere '{name}'?", "Conferma",
    ...
    private async Task SaveQuoteInfo()
    {
        ...
        {
            System.Diagnostics.Trace.WriteLine($"Error saving quote info: {ex.Message}");
        }
    }
    ...
    private async void BtnServiceRemoveProduct_Click(object sender, RoutedEventArgs e)
    {
        ...
        if (ATEC.PM.Client.Controls.ShadcnMessageBox.Show($"Rimuovere '{name}' e tutte le sue varianti?", "Conferma",
    ...
    private async void ServiceVariantToggle_Changed(object sender, RoutedEventArgs e)
    {
        ...
        {
            System.Diagnostics.Trace.WriteLine($"Error toggling service variant: {ex.Message}");
        }
    }
    ...
    private async void ServiceVariantField_LostFocus(object sender, RoutedEventArgs e)
    {
        ...
        {
            System.Diagnostics.Trace.WriteLine($"Error updating service variant field: {ex.Message}");
        }
    }
    ...
    private async Task ReloadServiceItems()
    {
        ...
        try
        {
            QuoteDto? quote = await ApiClient.GetDataAsync<QuoteDto>($"/api/quotes/{_selectedQuoteId}");
        ...
        {
            System.Diagnostics.Trace.WriteLine($"Error reloading service items: {ex.Message}");
        }
    }
    ...
    private async void ServiceDiscountPct_LostFocus(object sender, RoutedEventArgs e)
    {
        ...
        {
            System.Diagnostics.Trace.WriteLine($"Error updating discount percentage: {ex.Message}");
        }
    }
    ...
    private async void ServiceNotes_LostFocus(object sender, RoutedEventArgs e)
    {
        ...
        {
            System.Diagnostics.Trace.WriteLine($"Error updating service notes: {ex.Message}");
        }
    }
    ...
    private async void ServiceProductName_LostFocus(object sender, RoutedEventArgs e)
    {
        ...
        {
            System.Diagnostics.Trace.WriteLine($"Error updating service product name: {ex.Message}");
        }
    }
    ...
    private async void BtnServiceRefreshProduct_Click(object sender, RoutedEventArgs e)
    {
        ...
        if (ATEC.PM.Client.Controls.ShadcnMessageBox.Show("Aggiornare il prodotto e le varianti dal catalogo? Le modifiche locali verranno sovrascritte.",
            "Conferma", MessageBoxButton.YesNo, MessageBoxImage.Question) != MessageBoxResult.Yes) return;
    ...
    private async void BtnServiceCloneProduct_Click(object sender, RoutedEventArgs e)
    {
        ...
        if (ATEC.PM.Client.Controls.ShadcnMessageBox.Show("Duplicare questo prodotto con tutte le varianti?",
            "Conferma", MessageBoxButton.YesNo, MessageBoxImage.Question) != MessageBoxResult.Yes) return;
