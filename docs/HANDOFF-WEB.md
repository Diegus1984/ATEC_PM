...
- **Commerciale → Cat. Preventivi (Fase D2)** — albero listini→gruppi→categorie→prodotti→varianti (accordion, natural-sort, ricerca), CRUD gruppo/categoria/sotto-categoria/prodotto, sposta drag&drop, tabella prodotti (varianti espandibili, filtri colonna jolly, range prezzo/costo, auto-include), **editor descrizione TinyMCE 8 da npm** (`src/lib/tinymce.ts` è l'unico modulo che lo importa, chunk Vite caricato alla prima apertura, `license_key: 'gpl'`, config in `components/shared/RichTextEditor.tsx`; upload immagini su `/api/quote-catalog/products/upload`, path relativi `/uploads/cms/`). *Aggiornato dalla 5.10.9 vendorizzata (fuori supporto) il 04/09/2026: build/tsc/eslint/vitest OK, provato a runtime il 04/09/2026 nei due dialoghi, catalogo e preventivo: scrivere, incollare un'immagine (upload automatico), salvare, riaprire; tabella 50/50 e immagini esistenti intatte. Manca import Excel.*

- **Commerciale → Andamento** (`/andamento`, 04/09/2026) — il cruscotto con cui il venditore riferisce alla proprietà, metà **offerte**: 8 riquadri (emesse, prese con lo spacco a corpo/consuntivo, aperte, conversione, portafoglio ponderato, numeri riservati, righe senza importo, prese incomplete), cinque anni a confronto (linee), mese per mese (barre emesse/prese/aperte), aperte per livello di chance, portafoglio negli ultimi 12 mesi dal registro modifiche, analisi di mercato per categoria, per venditore, per tipo, primi dieci clienti e le quindici aperte più grandi — ogni grafico con la sua tabella accanto. Filtri anno/venditore/categoria, realtime su `sales-offers-all`. Gli euro seguono `data.revenue`: **li toglie il server**, non il client. I conti stanno in `AndamentoOfferteCalcolo` (puro, 22 test) e ricalcano `APP.stats` del vecchio applicativo, esclusioni comprese: le bozze non sono offerte emesse e le righe senza importo non entrano nei valori. **La palette è validata** (`validate_palette.js` della skill dataviz: tutti PASS in chiaro e scuro). 🪤 La legenda di recharts v3 si riordina da sola in ordine alfabetico e non accetta un `payload`: qui la disegna la pagina, sopra il grafico. 🪤 Sul 2025 la conversione esce 95,7% perché nel vecchio Excel le perse non si chiudevano — la pagina lo **dichiara** con un avviso invece di far finta di niente. *Build server 0/0 · `tsc -b` + `npm run build` + eslint OK · **runtime verificato il 04/09/2026** sui dati veri.* Piano: [piani/PIANO-ANDAMENTO-COMMERCIALE.md](piani/PIANO-ANDAMENTO-COMMERCIALE.md).

- **Commerciale → Registro Offerte** (`/offerte`, 04/09/2026, migrazioni **v122** e **v123**) — sostituisce l'Excel «NUMERI OFFERTE» e l'applicativo autonomo *ATEC Offerte*. Serie di numerazione **parallela** ai Preventivi (`S097-2026-EC` = tipo+progressivo+anno+sigla venditore, con `is_legacy_series` che esclude dal progressivo le 9 righe 2026 che proseguono la serie 2025), agganciabile a preventivo (`quote_id`) e commessa (`project_id`). Griglia paginata lato server con filtri anno/stato/venditore/tipo, ricerca, menu «Colonne» a chiave versionata, ordinamento, azioni riga (scheda, contatto, duplica, copia percorsi NAS, elimina); scheda con anteprima del numero composto, forbice di prezzo, esito+chance, ordine a corpo o a consuntivo, anticipo, percorsi, contatti col referente e storico modifiche; dialogo «Contatto col referente» che alimenta il prossimo richiamo. Realtime su `sales-offers-all`. **Storico importato**: 1.795 offerte dal 2022, 1.121 collegate alla rubrica, 16 clienti nuovi (`POST /api/sales-offers/import-seed`, simula per default). 🪤 I percorsi NAS si **copiano**, non si aprono: dal browser in HTTP un file di rete non è raggiungibile — è l'unica cosa che il vecchio applicativo faceva meglio, perché aveva un avviatore locale. *Build server 0/0 + 712 test verdi · `tsc -b` + `npm run build` + eslint + 68 vitest OK · **runtime GUI verificata il 04/09/2026** sui 1.795 record veri (elenco, filtri, colonne, scheda, crea/modifica/elimina, contatto, diretta SignalR provata da fuori) — 4 difetti trovati e corretti, dati di prova ripuliti.* Piano: [piani/PIANO-ANDAMENTO-COMMERCIALE.md](piani/PIANO-ANDAMENTO-COMMERCIALE.md).
...
✅ **Risorse/Gantt — sostanzialmente completo (port da `ATEC.Risorse.Web` Blazor, non dal WPF), verificato runtime 01/07/2026.** Gantt completo (header mesi/giorni, lane-packing, barre OP/FLEX/FERIE, weekend/festività/oggi, conflitti), filtri + zoom + pannello risorse + filtro "In lista" (persistenza localStorage), CRUD via `AssignmentDialog` (crea multi/modifica/duplica/elimina), **drag/resize/create-by-drag** (auto-pan, snap settimana, concorrenza ottimistica con messaggio 409 dedicato), scorciatoie tastiera (Ctrl+F/Canc), Shift+rotella, stampa Gantt, collassa pannello/colonna nomi, **presenza online** (pallino verde/rosso via SignalR), **dashboard Piano ferie** (`/risorse/ferie`), **digest email** (badge "modifiche da notificare", dialog "Notifica subito", pannello admin Config SMTP+scheduler in Gestione avanzata → Digest Email). File in `src/features/risorse/` + `src/features/admin/digest/`, API `src/lib/api/resource-planner.ts`+`digest.ts`+`settings.ts`. **Resta solo**: editor anagrafiche Service/Altre attività (bassa priorità, non blocca il flusso). **Sincronizzazione col programma ATEC Risorse sul VPS** (02/09/2026, Fase 0 fatta): scheda «Sincronizzazione ATEC Risorse (VPS)» in fondo a Digest Email (`features/admin/digest/RisorseSyncCard.tsx`, API `lib/api/risorse-sync.ts`) + motore `Services/RisorseSync/` lato server; piano e contratto in `docs/piani/PIANO-SYNC-RISORSE.md`. **03/09/2026 (#147, #148)**: barra ambra `features/risorse/SyncVpsWarning.tsx` sopra la toolbar del planner quando il VPS non risponde da > 10 min (`GET sync/salute`, chiave della pagina); le allocazioni assegnate arrivano nella **campanella** del dipendente (`RES_ASSIGNMENT`, `notification-navigation.ts` → `/risorse?alloc=ID`: il planner allenta i filtri, seleziona la barra e scorre a riga e periodo — `data-emp-id` sulle righe del Gantt, `scrollToDate` nel viewport).
...
✅ **HR → Timbrature, ferie e permessi** (dal 27/08/2026; punto d'ingresso `docs/piani/PIANO-HR-PRESENZE.md`, §7 per lo stato) — `/hr/timbrature` è una pagina con cinque viste-rotte (`features/hr/TimbraturePage.tsx`: **Controllo di ieri** sul percorso del gruppo, dal 09/09/2026, con le timbrature di ieri di tutti — di lunedì venerdì, sabato e domenica — e sollecito sulla riga o in blocco; cartellino di una persona `/hr/timbrature/cartellino`; tutti mese per mese; ore sulle commesse; email inviate — con la sola lettura si vede solo il proprio cartellino) più `/hr/richieste` (`RichiestePage.tsx`). Le celle della griglia stanno in `celle-cartellino.tsx` + `ore.ts`, i comandi della giornata in `AzioniGiornata.tsx`, le parole di «Com'è la giornata» in `stato-giornata.tsx`; la regola dei giorni del controllo è del server (`Services/Hr/HrControlloGiornaliero.cs`). API in `lib/api/hr.ts`, tipi in `lib/api/types/hr.ts`, real-time con `useHrHub`.
...
9. **Export tabellari = Excel vero** (08/09/2026, segnalazione #150): una griglia che si esporta
   passa da `downloadXlsx()` di `src/lib/api/export.ts` — colonne tipizzate (`testo`, `intero`,
   `numero`, `euro`, `percento`, `data`) + righe coi VALORI dei DTO — e il foglio `.xlsx` lo compone
   il server (`POST /api/export/xlsx`, `Services/Export/TabellaExcel.cs`, EPPlus): numeri, importi,
   percentuali e date come valori, filtro automatico su ogni colonna, intestazione bloccata.
   **Niente CSV con numeri dentro**: Excel italiano legge `27122.172` come ventisette milioni.
   Gli export `.xls` a tabella HTML (MoM, Sintesi DDP) e SpreadsheetML (Gantt) sono precedenti e
   si migrano quando si tocca la pagina. La tabella si costruisce in un modulo puro con il suo test
   (es. `features/sal/sal-prospetto-excel.ts`).
