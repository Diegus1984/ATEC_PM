...
- **PM → Pannello Lavorazioni** (`/lavorazioni`, prototipo V17; tabella `project_work_requests`, migrazione v20) — PmSidebar con viste Bozze (Staging)/Tabella Priorità/Controllo Consegne/Trattamenti + commesse; griglia editabile inline (`ProjectWorkRequests`, riusata nel dettaglio commessa), realtime `workrequests-all` + `row_version`. **Compilazione automatica dalla DDP Officina** (14/07/2026): **migrazione v26** = colonna `ddp_officina_item_id` (UNIQUE, FK SET NULL) + backfill; ogni riga officina genera/riallinea la lavorazione collegata (`WorkRequestDdpSync` — bozza in staging alla creazione; descrizione «Codice 101 — Descrizione · Q pz · Materiale», data disponibilità e flag trattamento **sempre allineati** alla riga DDP anche dopo la promozione; note trattamento e fornitore ODA precompilati solo alla creazione); delete riga DDP → bozza eliminata, promosse scollegate; **ritorno di stato**: lavorazione consegnata → riga officina a `COS` con evento `DdpChanged`. UI: badge «DDP» sulla descrizione, descrizione/data disponibilità/trattamento in sola lettura sulle collegate. *Server compila, tsc/eslint/vite OK; runtime GUI da verificare.*
